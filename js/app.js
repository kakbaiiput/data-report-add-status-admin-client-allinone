...
class AppsScriptAPI {
    ...
    async validateKitMulti(kitNumber, selectedMonth = null, selectedYear = null) {
        // ⚡ Try VPS MySQL first — typically 50-200ms vs 5-30s for GAS JSONP
        try {
            const url = new URL(VPS_VALIDATE_URL);
            url.searchParams.set('action', 'validate');
            url.searchParams.set('kit', kitNumber);
            ...
            if (resp.ok) {
                ...
                if (data && data.validation) {
                    CONFIG.log('⚡ VPS validate hit:', kitNumber, data.validation.status);
                    return data;
                }
            }
        } catch (vpsErr) {
            CONFIG.log('⚠️ VPS validate unavailable, falling back to GAS:', vpsErr.message);
        }
...
function handleSubmitError(error) {
    ...
    setTimeout(() => {
        ...
        if (error.message.includes('duplicate') || error.message.includes('Duplicate') || error.message.includes('already')) {
            ...
            resetFormAfterSuccess();
        } else if (error.message.includes('timeout') || error.message.includes('JSONP') || error.message.includes('script error')) {
            showBanner('⚠️ Koneksi timeout. Data kemungkinan sudah tersimpan — cek laporan sebelum submit ulang.', 'error');
            resetFormAfterSuccess();
        } else {
            showBanner(errorMessage, 'error');
        }
